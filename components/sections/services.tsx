...
interface ServicesProps {
  compact?: boolean
  showTeaserButton?: boolean
  hideHeader?: boolean
}

export function Services({ 
  compact = false, 
  showTeaserButton = true,
  hideHeader = false 
}: ServicesProps) {
  return (
    <section className={cn(
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        {/* ETAP 1: Fundament - Hidden in compact mode */}
        {!compact && (
          <div className={cn(hideHeader ? "mt-0" : "mt-16 lg:mt-24")}>
        ...
        {/* ETAP 2: Rozwiązania */}
        <div className={cn(compact ? "mt-16" : "mt-24")}>
          <div className="flex items-center gap-4 mb-12">
            <div className="h-px flex-grow bg-primary/20" />
            <span className="text-sm font-bold text-primary uppercase tracking-widest px-4 py-1 rounded-full border border-primary/20 bg-primary/5">
              {compact ? "Główne obszary pomocy" : "ETAP 2: Rozwiązania (Wdrożenie)"}
            </span>
            <div className="h-px flex-grow bg-primary/20" />
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {pillars.map((pillar) => (
              <Link key={pillar.id} href={pillar.href} className="flex flex-col group">
                <Card 
                  className="flex flex-col flex-grow border-primary/10 transition-all duration-300 hover:border-primary/40 hover:shadow-xl hover:-translate-y-1 bg-background group-hover:bg-primary/[0.02]"
                >
                  <CardHeader>
                    <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-primary/10 group-hover:bg-primary transition-colors duration-300">
                      <pillar.icon className="h-6 w-6 text-primary group-hover:text-primary-foreground transition-transform duration-300 group-hover:scale-110" />
                    </div>
                  ...
                  <CardContent className="flex flex-grow flex-col gap-4 text-muted-foreground">
                    ...
                    <div className="mt-auto pt-6 flex flex-col gap-4">
                      ...
                      <div className="flex items-center gap-2 text-xs font-bold text-primary uppercase tracking-wider group-hover:gap-3 transition-all">
                        Sprawdź szczegóły usługi <ArrowRight className="h-3 w-3" />
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        {/* ETAP 3: Wsparcie - Hidden in compact mode */}
        {!compact && (
          <div className="mt-24">
            <div className="flex items-center gap-4 mb-12">
              <div className="h-px flex-grow bg-primary/20" />
          ...
          <div className="mt-24 grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-3xl bg-muted p-8 sm:p-12 border border-border flex flex-col h-full">
              ...
              <p className="mt-4 text-lg text-muted-foreground text-balance">
                Moje rozwiązania są inwestycją in trwałą zmianę efektywności Twojej firmy. Nie współpracuję z firmami, które:
              </p>
